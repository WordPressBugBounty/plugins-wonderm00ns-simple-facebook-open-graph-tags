 ...
 *
 * This file contains the Webdados_FB_Admin class which handles all admin-related
 * functionality including settings pages, meta boxes, option validation, admin notices,
 * and integration with WordPress admin interface.
 *
...
if ( ! class_exists( 'Webdados_FB_Admin' ) ) :
	/**
	 * Admin functionality class.
	 *
	 * Handles all admin-related functionality including settings pages, meta boxes on posts/pages,
	 * option validation and sanitization, admin notices, and integration with WordPress admin interface.
	 * This class manages the plugin's admin UI and user interactions.
	 *
	 * @package Wonderm00ns_Simple_Facebook_Open_Graph_Tags
	 * @subpackage Admin
	 * @since    1.0.0
	...
	class Webdados_FB_Admin {
		 ...
		 *
		 * @since 1.0.0
		 * @var bool $update_status Update FB status
		 */
		 ...
		 *
		 * @since 1.0.0
		 * @var string $update_error Update error message.
		 */
		 ...
		 *
		 * @since 1.0.0
		 * @var bool $update_show Whether to show update notice.
		 */
		private $update_show;
		 ...
		 * Constructor.
		 *
		 * Initializes the admin class with options and version.
		 *
		 * @since 1.0.0
		...
		/**
		 * Create admin menu and register hooks.
		 *
		 * Adds the plugin settings page to WordPress admin menu under Settings,
		 * and registers admin styles, scripts, and notices.
		 *
		 * @since 1.0.0
		 */
		public function create_admin_menu() {
			$options_page = add_options_page( WEBDADOS_FB_PLUGIN_NAME, WEBDADOS_FB_PLUGIN_NAME, 'manage_options', 'ogatc-settings', array( $this, 'options_page' ) );
		...
		/**
		 * Initialize and register plugin settings.
		 *
		 * Registers the plugin settings with WordPress Settings API and
		 * associates the validation callback.
		 *
		 * @since 1.0.0
		 */
		 ...
		 *
		 * Registers plugin option strings with WPML for translation when
		 ...
		 *
		 * Adds a "Settings" link to the plugin's row on the WordPress plugins page.
		 *
		 * @since 1.0.0
		 ...
		 *
		 * Adds Google+, Twitter, and Facebook profile fields to user contact methods,
		 * unless Yoast SEO is active (which already provides these fields).
		 *
		 * @since 1.0.0
		 * @param array $usercontacts Array of existing user contact methods.
		 * @return array Modified array with social media fields added.
		 */
		...
		/**
		 * Get post types that support Open Graph meta boxes.
		 *
		 * Returns an array of public post types that should display the Open Graph
		 * meta box, excluding attachment and other filtered types.
		 *
		 * @since 1.0.0
		...
		public function get_post_types() {
			...
			if ( ! isset( $public_types['page'] ) ) {
				$public_types['page'] = 'page';
			}
		 ...
		 *
		 * Displays stored admin notices (e.g., from tool operations) and clears them.
		 *
		 * @since 1.0.0
		 */
		public function admin_notice() {
			$admin_notice = get_option( 'wonderm00n_open_graph_admin_notice' );
			if ( $admin_notice ) {
		...
		/**
		 * Add Open Graph meta boxes to post edit screens.
		 *
		 * Registers meta boxes for Open Graph settings on supported post types.
		 *
		 * @since 1.0.0
		 * @param array $usercontacts Unused parameter (required by WordPress hook).
		 */
		...
		/**
		 * Render Open Graph meta box content.
		 *
		 * Outputs the HTML form fields for post-specific Open Graph settings
		 * including title, image, and description overrides.
		 *
		 * @since 1.0.0
		 */
				...
				<p>
					<strong>
						<label for="webdados_fb_open_graph_specific_title">
							<?php esc_html_e( 'Use this title', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>:
						</label>
					</strong>
					<br/>
					...
					<br/>
					<?php esc_html_e( 'If this field is not filled, the title will be generated from the post title', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
				</p>
				<?php
			}
				...
				<p>
					<strong>
						<label for="webdados_fb_open_graph_specific_image">
							<?php esc_html_e( 'Use this image', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>:
						</label>
					</strong>
					<br/>
					...
					printf(
						...
						esc_attr__( 'Recommended size: %1$dx%2$dpx', 'wonderm00ns-simple-facebook-open-graph-tags' ),
						...
						jQuery(document).ready(function($){
							...
							$('#webdados_fb_open_graph_specific_image_button').click(function(e){
								...
								meta_image_frame = wp.media.frames.meta_image_frame = wp.media({
									title: "<?php esc_html_e( 'Select image', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>",
									button: { text:  "<?php esc_html_e( 'Use this image', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>" },
									library: { type: 'image' }
								});
				...
				<p>
					<strong>
						<label for="webdados_fb_open_graph_specific_description">
							<?php esc_html_e( 'Use this description', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>:
						</label>
					</strong>
					<br/>
					<?php
					if ( $webdados_fb->is_yoast_seo_active() && ( 1 == $this->options['fb_show_wpseoyoast'] ) ) {
						esc_html_e( 'The Yoast SEO integration is active, so it\'s description will be used', 'wonderm00ns-simple-facebook-open-graph-tags' );
					} else {
						?>
						...
						<br/>
						<?php
						esc_html_e( 'If this field is not filled, the description will be generated from the excerpt, if it exists, or from the content', 'wonderm00ns-simple-facebook-open-graph-tags' );
					}
					?>
				</p>
				<?php
		 ...
		 *
		 * Validates and saves post-specific Open Graph settings (title, image, description)
		 * and optionally triggers Facebook cache update.
		 *
		 * @since 1.0.0
		...
		public function save_meta_boxes( $post_id ) {
			...
			$nonce = ( isset( $_POST['webdados_fb_open_graph_custom_box_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['webdados_fb_open_graph_custom_box_nonce'] ) ) : '' );
			...
			if ( $save ) {
				...
				if ( isset( $_POST['webdados_fb_open_graph_specific_image'] ) ) {
					// Sanitize user input.
					$mydata = trim( sanitize_text_field( wp_unslash( $_POST['webdados_fb_open_graph_specific_image'] ) ) );
				...
				if ( isset( $_POST['webdados_fb_open_graph_specific_description'] ) ) {
					...
					if ( get_bloginfo( 'version' ) >= '4.7.0' ) { // sanitize_textarea_field only exists since 4.7.0.
						$mydata = trim( sanitize_textarea_field( wp_unslash( $_POST['webdados_fb_open_graph_specific_description'] ) ) );
					} else {
						// Just in case...
						$mydata = trim( sanitize_text_field( wp_unslash( $_POST['webdados_fb_open_graph_specific_description'] ) ) );
					}
					// Update the meta field in the database.
				...
				if ( isset( $_POST['webdados_fb_open_graph_specific_title'] ) ) {
					// Sanitize user input.
					$mydata = trim( sanitize_text_field( wp_unslash( $_POST['webdados_fb_open_graph_specific_title'] ) ) );
					// Update the meta field in the database.
		...
		/**
		 * Add Facebook update status to redirect URL.
		 *
		 * Appends query parameters to post edit redirect URL to indicate
		 * Facebook cache update status and any errors.
		 *
		 * @since 1.0.0
		 * @param string $location Redirect URL.
		 * @param int    $post_id  Post ID.
		 * @return string Modified redirect URL with status parameters.
		 */
		public function redirect_post_location( $location, $post_id ) {
			$location = add_query_arg( 'wd_fb_og_status', $this->update_status, $location );
			if ( $this->update_error ) {
				$location = add_query_arg( 'wd_fb_og_error', urlencode( $this->update_error ), $location );
			}
			return $location;
		}
		/**
		 * Display Facebook cache update status notices.
		 *
		 * Shows success or error notices after attempting to update Facebook's
		 * Open Graph cache when a post is saved.
		 *
		 * @since 1.0.0
		 */
		public function admin_notices() {
			if ( intval( $this->options['fb_adv_supress_fb_notice'] ) == 0 ) {
						...
						if ( 'edit' === $screen->parent_base && 'post' === $screen->base ) {
							...
							switch ( intval( $_GET['wd_fb_og_status'] ) ) {
								case '1':
									?>
									<div class="updated">
										<p><?php esc_html_e( 'Facebook Open Graph Tags cache updated/purged.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?> <a class="button button-small" style="margin: 0px 1em;" href="https://www.facebook.com/sharer.php?u=<?php echo urlencode( get_permalink( $post->ID ) ); ?>" target="_blank"><?php esc_html_e( 'Share this on Facebook', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?></a></p>
									</div>
									<?php
									break;
								case '-1':
								...
								case '-3':
									...
									<div class="error">
										<p>
										<?php
											echo '<strong>' . esc_html__( 'Error: Facebook Open Graph Tags cache NOT updated/purged.', 'wonderm00ns-simple-facebook-open-graph-tags' ) . '</strong>';
										if ( isset( $_GET['wd_fb_og_error'] ) ) {
											echo '<br/>' . esc_attr( sanitize_text_field( wp_unslash( $_GET['wd_fb_og_error'] ) ) );
										}
										?>
										</p>
										<p>
											<strong>
												<?php esc_html_e( 'This is NOT a plugin error.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
											</strong>
											<br/>
											- <?php esc_html_e( 'Do not open support tickets about this issue.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
											- <?php esc_html_e( 'Lately and unfortunately, Facebook is not allowing to update the cache programmatically.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
											<br/>
											- <?php esc_html_e( 'Have you already configured the App ID and App Secret, needed for flushing the cache on Facebook?', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
											<a href="options-general.php?page=<?php echo esc_attr( basename( __FILE__ ) ); ?>"><?php esc_html_e( 'If you haven\'t, go to the settings page (Open Graph tab), and do it now.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?></a>
											<br/>
											or
											<br/>
											- <a href="https://developers.facebook.com/tools/debug/sharing/?q=<?php echo urlencode( get_permalink( $post->ID ) ); ?>" target="_blank"><?php esc_html_e( 'Click here to try to clear the cache manually and then click "Scrape Again"', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?></a>
										</p>
									</div>
									<?php
									break;
				 ...
				 * ?>
				 * <div class="updated">
				 * <p><?php esc_html_e('Facebook Open Graph Tags cache updated/purged.', 'wonderm00ns-simple-facebook-open-graph-tags'); ?> <a href="https://www.facebook.com/sharer.php?u=<?php echo urlencode(get_permalink($post->ID));?>" target="_blank"><?php esc_html_e('Share this on Facebook', 'wonderm00ns-simple-facebook-open-graph-tags'); ?></a></p>
				 * </div>
				 * <?php
				 * } else {
		...
		/**
		 * Add manual Facebook cache update links to post update messages.
		 *
		 * Adds buttons to post update messages for manually updating Facebook cache
		 * and sharing posts on Facebook.
		 *
		 * @since 1.0.0
		 * @param array $messages Array of post update messages.
		 * @return array Modified array with Facebook cache update links added.
		 */
		 ...
		 *
		 * Loads and displays the main settings page template.
		 *
		 * @since 1.0.0
		 */
		public function options_page() {
			$options = $this->options;
			require_once plugin_dir_path( __DIR__ ) . 'admin/options-page.php';
		}
		/**
		 * Enqueue admin stylesheet.
		 *
		 * Registers and enqueues the admin CSS file for the settings page.
		 *
		 * @since 1.0.0
		 */
		 ...
		 *
		 * Registers and enqueues the admin JavaScript file with dependencies
		 * and localized strings for the settings page.
		 *
		 * @since 1.0.0
		 */
		public function admin_scripts() {
			wp_enqueue_script( 'webdados_fb_admin_script', plugins_url( 'js/webdados-fb-open-graph-admin.js', __FILE__ ), array( 'jquery', 'jquery-ui-tabs', 'media-upload' ), $this->version, false );
			wp_localize_script(
				'webdados_fb_admin_script',
				'texts',
				array(
		 ...
		 *
		 * Sanitizes all plugin options based on their defined sanitization methods
		 * (intval for integers, trim for strings).
		 *
		 * @since 1.0.0
		 * @param array $options Array of option values to validate.
		...
		/**
		 * Execute requested admin tools.
		 *
		 * Processes tool execution requests from the settings page and calls
		 * the corresponding tool methods.
		 *
		 * @since 1.0.0
		 * @param mixed $value Option value (unused, required by filter).
		 * @return mixed Unchanged option value.
		 */
		public function run_tools( $value ) {
			if ( isset( $_POST['tools'] ) && is_array( $_POST['tools'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				foreach ( $_POST['tools'] as $tool ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		/**
		 * Clear image size cache transients.
		 *
		 * Deletes all cached image size transients from the database
		 * and displays a success notice.
		 *
		 * @since 1.0.0
		 */
