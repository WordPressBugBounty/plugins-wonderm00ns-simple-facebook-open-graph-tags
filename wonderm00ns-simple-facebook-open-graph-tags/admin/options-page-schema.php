<?php
/**
 * Schema.org settings options page template.
 *
 * This file contains the HTML and form fields for the Schema.org settings tab
 * in the plugin options page, including Schema.org markup type selection and
 * Schema.org tag visibility options.
 *
...
<div class="menu_containt_div" id="tabs-4">
	<p><strong><?php esc_html_e( 'This feature is deprecated and will soon be removed.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?></strong></p>
	<p><?php esc_html_e( 'Schema.org tags used by Google+ to render link share posts.', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?></p>

	<?php do_action( 'fb_og_admin_settings_schema_before' ); ?>

	<div class="postbox">
		<h3 class="hndle"><i class="dashicons-before dashicons-googleplus"></i> <?php esc_html_e( 'Google+ / Schema.org Tags', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?></h3>
		<div class="inside">
				...
				<tbody>
					...
					<tr>
						<td colspan="2" class="info">
							<i>&lt;meta itemprop="name" content="..."/&gt;</i> <?php esc_html_e( 'and', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?> <i>&lt;meta itemprop="headline" content="..."/&gt;</i>
							<br/>
							-
							<?php
							printf(
					...
					<tr>
						<td colspan="2" class="info">
							...
							<br/>
							- <?php esc_html_e( 'Experimental', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
							<br/>
							- <?php esc_html_e( 'Added to the HTML tag, if you want to avoid W3C and Structured Data validation errors', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
							<br/>
							- 
							<?php
							printf(
					...
					<tr class="fb_type_schema_options">
						...
						<td>
							<select name="wonderm00n_open_graph_settings[fb_type_schema_homepage]" id="fb_type_schema_homepage">
								...
								<?php
								if ( trim( $options['fb_type_schema_homepage'] ) == '' || trim( $options['fb_type_schema_homepage'] ) == 'WebSite' ) {
									echo ' selected="selected"';}
								?>
								>WebSite</option>
								<option value="Blog"
								<?php
								if ( trim( $options['fb_type_schema_homepage'] ) == 'Blog' ) {
									echo ' selected="selected"';}
								?>
								>Blog</option>
							</select>
					...
					<tr class="fb_type_schema_options">
						<th><?php esc_html_e( 'Default (including Post/Page) Type', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>:</th>
						<td>
							<select name="wonderm00n_open_graph_settings[fb_type_schema_post]" id="fb_type_schema_post">
								<option value="Article"
								<?php
								if ( trim( $options['fb_type_schema_post'] ) == '' || trim( $options['fb_type_schema_post'] ) == 'Article' ) {
									echo ' selected="selected"';}
								?>
								>Article</option>
								<option value="NewsArticle"
								<?php
								if ( trim( $options['fb_type_schema_post'] ) == 'NewsArticle' ) {
									echo ' selected="selected"';}
								?>
								>NewsArticle</option>
								<option value="Report"
								<?php
								if ( trim( $options['fb_type_schema_post'] ) == 'Report' ) {
									echo ' selected="selected"';}
								?>
								>Report</option>
								<option value="ScholarlyArticle"
								<?php
								if ( trim( $options['fb_type_schema_post'] ) == 'ScholarlyArticle' ) {
									echo ' selected="selected"';}
								?>
								>ScholarlyArticle</option>
								<option value="TechArticle"
								<?php
								if ( trim( $options['fb_type_schema_post'] ) == 'TechArticle' ) {
									echo ' selected="selected"';}
								?>
								>TechArticle</option>
							</select>
					...
					<tr>
						<td colspan="2" class="info">
							...
							<br/>
							- <?php esc_html_e( 'The user\'s Google+ URL must be filled in on his profile', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
						</td>
					</tr>-->
					...
					<tr>
						<td colspan="2" class="info">
							<i>&lt;itemprop="datePublished" content="..."/&gt;</i> <?php esc_html_e( 'and', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?> <i>&lt;meta itemprop="dateModified" content="..."/&gt;</i>
							<br/>
					...
					<tr>
						<td colspan="2" class="info">
							...
							<br/>
							- <?php esc_html_e( 'The website\'s Google+ Page', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
						</td>
					</tr>
					
					<tr class="fb_publisher_schema_options">
						<th><?php esc_html_e( 'Website\'s Google+ Page', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>:</th>
						<td>
							<input type="text" name="wonderm00n_open_graph_settings[fb_publisher_schema]" id="fb_publisher_schema" size="50" value="<?php echo esc_attr( trim( $options['fb_publisher_schema'] ) ); ?>"/>
						</td>
					</tr>
					<tr class="fb_publisher_schema_options">
						<td colspan="2" class="info">
							- <?php esc_html_e( 'Google+ Page URL (with https://)', 'wonderm00ns-simple-facebook-open-graph-tags' ); ?>
						</td>
					</tr>
