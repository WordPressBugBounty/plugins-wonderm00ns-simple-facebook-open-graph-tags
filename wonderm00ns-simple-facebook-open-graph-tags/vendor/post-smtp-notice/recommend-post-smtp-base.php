...
class Recommend_Post_SMTP_Base {
    ...
    public function recommend_post_smtp_submenu() {
        ...
        // Check if Post SMTP is installed but not active
        if( ! function_exists( 'is_plugin_active' ) ) {
            require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
        }
        
        $post_smtp_plugin = 'post-smtp/postman-smtp.php';
        $is_installed = file_exists( WP_PLUGIN_DIR . '/' . $post_smtp_plugin );
        $is_active = is_plugin_active( $post_smtp_plugin );
